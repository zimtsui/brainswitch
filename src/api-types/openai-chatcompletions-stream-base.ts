...
import { OpenAIChatCompletionsAPIBase, InvalidFinishReason } from './openai-chatcompletions-base.ts';
import { type InferenceContext } from '../inference-context.ts';
import { RetryLimitError, TransientError } from './base.ts';
	...
	protected async stream(ctx: InferenceContext, session: Session<Function.Declaration.From<fdm>>, retry = 0): Promise<RoleMessage.AI<Function.Declaration.From<fdm>>> {
		...
		try {
			...
			assert(
				...
				new InvalidFinishReason(finishReason),
			);
