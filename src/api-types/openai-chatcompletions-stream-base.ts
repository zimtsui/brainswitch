...
export abstract class OpenAIChatCompletionsStreamAPIBase<in out fdm extends Function.Declaration.Map = {}> extends OpenAIChatCompletionsAPIBase<fdm> {
	...
	protected async stream(ctx: InferenceContext, session: Session<Function.Declaration.From<fdm>>, retry = 0): Promise<RoleMessage.AIClass<Function.Declaration.From<fdm>>> {
		...
		try {
			...
			return new RoleMessage.AIClass([
				new RoleMessage.Part.TextClass(this.extractContent(text)),
				...fcs,
			]);
		} catch (e) {
