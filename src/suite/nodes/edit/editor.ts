...
export class Editor {
	...
	public read({ begin, end }: { begin: number, end: number }): string {
	...
	public splice({ fragment, begin, end }: { fragment: string, begin: number, end: number }): string {
		const text = this.getText({});
	...
	public copy({ destOffset, srcBegin, srcEnd }: { destOffset: number, srcBegin: number, srcEnd: number }): string {
		const text = this.getText({});
	...
	private rotate({ text, begin, mid, end }: { text: string, begin: number, mid: number, end: number }): string {
		return text.slice(0, begin) + text.slice(mid, end) + text.slice(begin, mid) + text.slice(end);
	}

	public move({ destOffset, srcBegin, srcEnd }: { destOffset: number, srcBegin: number, srcEnd: number }): string {
		const text = this.getText({});
		if (this.located.has(srcBegin)) {} else return Editor.notLocated(srcBegin);
		if (this.located.has(srcEnd)) {} else return Editor.notLocated(srcEnd);
		if (this.located.has(destOffset)) {} else return Editor.notLocated(destOffset);
	...
	public find({ fragment }: { fragment: string }): string {
		const text = this.getText({});
	...
	public slice({ begin, end }: { begin: number, end: number }): string {
		const text = this.getText({});
		if (this.located.has(begin)) {} else return Editor.notLocated(begin);
		if (this.located.has(end)) {} else return Editor.notLocated(end);
	...
	public undo({ limit }: { limit: number }): string {
		let i = 0;
	...
	public throw({ reason }: { reason: string }): string {
		throw new Error(reason);
	}

	public submit({}: {}): string {
		throw new Error('编辑结束');
	}
}
